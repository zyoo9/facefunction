...
def get_frame_processors_modules(frame_processors : List[str]) -> Dict[str, ModuleType]:
	global FRAME_PROCESSORS_MODULES

	if not FRAME_PROCESSORS_MODULES:
		for frame_processor in frame_processors:
			frame_processor_module = load_frame_processor_module(frame_processor)
			FRAME_PROCESSORS_MODULES[frame_processor] = frame_processor_module
	part_of_modules = []
	for frame_processor in frame_processors:
		if FRAME_PROCESSORS_MODULES[frame_processor]:
			part_of_modules.append(FRAME_PROCESSORS_MODULES[frame_processor])
	return part_of_modules
...
def clear_frame_processors_modules() -> None:
	...
	if not FRAME_PROCESSORS_MODULES:
		return
	for frame_processor, frame_processor_module in FRAME_PROCESSORS_MODULES.items():
