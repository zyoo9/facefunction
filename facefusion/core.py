...
def get_args(mode : ArgumentsMode = 'all') -> ArgumentParser:
	# possible future using of program.add_argument(..., action='append') for 'batch-task' mode will broke the logic of batch-set-defaults in the current logic implementation
	...
	if mode in ('batch-common'):
		...
		program.add_argument('--batch-tasks-limit-before-release', help = wording.get('batch_release_resources_limit_help'), dest = 'batch_release_resources_limit', type = int, default = 0)
		program.add_argument('--batch-continue-from-line', help = wording.get('batch_continue_from_line_help'), dest = 'batch_continue_from_line', type = int, default = 0)
	if mode in ('all', 'batch-task'):
		# general
		program.add_argument('-s', '--source', help = wording.get('source_help'), dest = 'source_paths', nargs = '+')
	...
	if mode not in ('batch-task'):
		...
		group_execution.add_argument('--execution-providers', help = wording.get('execution_providers_help'), dest = 'execution_providers', default = [ 'mps' ], choices = encode_execution_providers(onnxruntime.get_available_providers()), nargs = '+')
...
def batch_cli() -> None:
	...
	run(program, is_batch_mode = True)
...
def apply_args(program : ArgumentParser, mode : ArgumentsMode = 'all', raw_args : Optional[List[str]] = None) -> Namespace:
	...
	if mode in ('all', 'batch-task'):
		...
		# facefusion.globals.output_path = "/Users/zyooo/Desktop/test"
...
def run(program : ArgumentParser, is_batch_mode : bool = False) -> None:
	args_mode = 'all' if not is_batch_mode else 'batch-common'
	parsed_args = apply_args(program, args_mode) # type: ignore
	limit_resources()
...
def batch_process(common_args : Namespace) -> None:
	# 检查文件
	if not os.path.isfile(common_args.batch_file_path):
		update_status(wording.get('batch_file_bad_path'))
		return
	with open(common_args.batch_file_path, 'r') as f:
		batch_lines = list(line.strip() for line in f)
	if len(list(filter(None, batch_lines))) == 0:
		update_status(wording.get('batch_file_empty'))
		return

	program = get_args('batch-task')
	program.add_argument('--batch-output-prefix', dest = 'batch_output_prefix', default = '')
	program.add_argument('--frame-processors', dest = 'frame_processors', default = facefusion.globals.frame_processors, choices = facefusion.globals.frame_processors, nargs = '+')

	batch_task_defaults = ''
	tasks_counter = common_args.batch_release_resources_limit
	print("tasks_counter：", tasks_counter)

	for i, args_line in enumerate(batch_lines):
		args_line = args_line.split('#', 1)
		args_line = args_line[0].rstrip()
		if args_line == '':
			continue
		if args_line.startswith('batch-clear-defaults'):
			batch_task_defaults = ''
			args_line = args_line.replace('batch-clear-defaults', '', 1)
			if args_line == '':
				continue
		if args_line.startswith('batch-set-defaults'):
			batch_task_defaults = args_line.replace('batch-set-defaults', '', 1)
			print("batch_task_defaults：", batch_task_defaults)
			continue

		if i < common_args.batch_continue_from_line:
			continue
		update_status(wording.get('batch_line_started').format(line_id = i))
		args_line = batch_task_defaults + ' ' + args_line
		print("args_line：", args_line)

		task_args = apply_args(program, 'batch-task', args_line.split())
		print("task_args：", task_args)
		facefusion.globals.frame_processors = task_args.frame_processors
		batch_output_prefix = task_args.batch_output_prefix.lstrip(os.pathsep)
		print("batch_output_prefix：", batch_output_prefix)
		if facefusion.globals.output_path and batch_output_prefix != '':
			print("进if")
			output_path_pair = os.path.split(facefusion.globals.output_path)
			facefusion.globals.output_path = os.path.join(output_path_pair[0], batch_output_prefix + output_path_pair[1])

		tasks_counter -= 1
		print("tasks_counter：", tasks_counter)
		if tasks_counter <= 0 or i >= len(batch_lines) - 1:
			tasks_counter = common_args.batch_release_resources_limit
			conditional_process(is_warm_process = False)
		else:
			conditional_process(is_warm_process = True)
		clear_faces_cache()
	update_status(wording.get('batch_processing_ended'))


def conditional_process(is_warm_process : bool = False) -> None:
	conditional_set_face_reference()
	for frame_processor_module in get_frame_processors_modules(facefusion.globals.frame_processors):
		if not frame_processor_module.pre_process('output'):
			return
	if is_image(facefusion.globals.target_path):
		process_image(is_warm_process)
	if is_video(facefusion.globals.target_path):
		process_video(is_warm_process)
...
def process_image(is_warm_process : bool = False) -> None:
	...
	for frame_processor_module in get_frame_processors_modules(facefusion.globals.frame_processors):
		...
		frame_processor_module.post_process(is_warm_process)
	# compress image
...
def process_video(is_warm_process : bool = False) -> None:
